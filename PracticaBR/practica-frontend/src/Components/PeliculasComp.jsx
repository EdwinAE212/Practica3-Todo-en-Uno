...
import PeliculaModal from "../Modals/PeliculaModal";
...
function PeliculasComp() {
  ...
  const cargarPagina = async (page, query = "") => {
    try {
      const res = await getPeliculas(page, registrosPorPagina, query);

      setPeliculas(Array.isArray(res.data) ? res.data : []);
      ...
      if (res.totalGeneral !== undefined) {
        ...
        if (!query) {
          ...
          try {
            const resGlobal = await getPeliculas(1, 1, "");
      ...
      mostrarToast("Error al cargar películas", "error");
    }
  };

  const guardarPelicula = async (valores) => {
    try {
      if (valores._id) {
        await axios.put(`${API_BASE_URL}/peliculas/${valores._id}`, valores);
        mostrarToast("Película editada con éxito", "success");
      } else {
        await axios.post(`${API_BASE_URL}/peliculas`, valores);
        mostrarToast("Película agregada con éxito", "success");
      }
  ...
  const eliminarPelicula = async () => {
    try {
      await axios.delete(`${API_BASE_URL}/peliculas/${peliculaSeleccionada._id}`);
      mostrarToast("Película eliminada con éxito", "success");
      setConfirmarVisible(false);
      cargarPagina(pagina, busqueda);
    } catch (error) {
      console.error("Error eliminando película", error);
      mostrarToast("Error al eliminar película", "error");
    }
  };
    ...
    <div className="p-6 w-full bg-gradient-to-b bg-[D8D7D533]">
      <div className="bg-white rounded-2xl shadow-md shadow-gray-300 mb-6 p-6 flex justify-between items-center border border-gray-200 w-full max-w-7xl mx-auto">
        <div className="flex items-center gap-3">
          <FilmIcon className="w-9 h-9 text-blue-800" />
              ...
              {peliculas.length > 0 ? (
                peliculas.map((p, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">{p.director}</td>
                    <td className="px-4 py-3">{p.producer}</td>
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <div className="flex items-center bg-gray-200 rounded-4xl shadow border border-gray-300 px-2">
            <button
              onClick={() => setPagina(1)}
              disabled={pagina === 1}
              className="p-2 rounded-l-4xl transition hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={pagina === 1}
              className="p-2 transition hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={pagina === totalPaginas}
              className="p-2 rounded-r-4xl transition hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
