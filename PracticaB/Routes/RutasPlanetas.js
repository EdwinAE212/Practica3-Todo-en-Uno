...
router.get("/", async (req, res) => { 
    try {
        ...
        const total = await PlanetaServ.Contar(search);
        const lista = await PlanetaServ.ListaTodos(page, limit, search);
        const totalGeneral = await PlanetaServ.Contar("");
...
router.get("/personaje/:name", async (req, res) => {
    ...
    let name = req.params.name;
        ...
        res.json(await PlanetaServ.ListaPersonaje(name));
    } catch (e) {
        res.status(400).json({ error: e.message });
    }
});
