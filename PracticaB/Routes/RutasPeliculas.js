...
router.get("/", async (req, res) => { 
    try {
        ...
        const total = await PeliculaServ.Contar(search);
        const lista = await PeliculaServ.ListaTodos(page, limit, search);
        const totalGeneral = await PeliculaServ.Contar("");
...
router.get("/personaje/:name", async (req, res) => {
    ...
    let name = req.params.name;
        ...
        res.json(await PeliculaServ.ListaPersonaje(name));
    } catch (e) {
        res.status(400).json({ error: e.message });
    }
});
