...
router.get("/", async (req, res) => { 
    try {
        ...
        const total = await EspecieServ.Contar(search);
        const lista = await EspecieServ.ListaTodos(page, limit, search);
        const totalGeneral = await EspecieServ.Contar("");
...
router.get("/personaje/:name", async (req, res) => {
    ...
    let name = req.params.name;
        ...
        res.json(await EspecieServ.ListaPersonaje(name));
    } catch (e) {
        res.status(400).json({ error: e.message });
    }
});
